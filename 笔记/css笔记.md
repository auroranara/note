...
### 浮动与清除浮动
>浮动元素会一直浮动到包含框或另一个浮动框为止，并且浮动元素是脱离文档流的；

>浮动元素只会影响内联框（通常是文本）的排列。（如果浮动元素排在块级元素之前会覆盖块级元素）
...
* 闭合浮动
	>本质就是让父级元素变成BFC(见前文)或者hasLayout属性为true
	触发hasLayout的条件只要记：最无害和被大家用来处理旧版本IE闭合浮动兼容性的是zoom（不为normal）。
* 清除浮动
