...
* this的指向问题？[参考](https://juejin.im/post/59bfe84351882531b730bac2#heading-8)
    ...
    * 改变this的几种：
        ...
        * 内部_this=this
        * call、apply、bind
            > apply和call类似，传入参数不同（fun.apply( this,[] )），bind创建新的函数，必须手动调用（ a.bind(this,1,2)() ）
        * new实例化一个对象 
...
* Promise相关 [参考](https://juejin.im/post/5aa7868b6fb9a028dd4de672) , [面试官眼中的promise](https://juejin.im/post/5c233a8ee51d450d5a01b712)
    ...
    * then中onReject和.catch谁在前用谁，只会触发一次。
    * promise.finally(fn)无论promise是fulfilled还是rejected都被调用，不接受参数，返回上一个promise值包装成的新promise
